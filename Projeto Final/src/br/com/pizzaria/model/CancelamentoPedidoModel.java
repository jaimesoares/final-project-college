...
//                + ";";
        try (PreparedStatement pstmt = ConectaBanco.getConnection().prepareStatement(SqlSelection)) {
            ...
            while (rs.next()) {
                tabela.addRow(new Object[]{rs.getInt("ped_cod"), "Entrega", rs.getString("cli_nome"), rs.getString("cli_telefone"), rs.getString("ped_pagamento"), rs.getString("ped_vlr_tot"), rs.getString("ped_stt_canc")});
            }
...
//        }
    ...
    public void listarPedidoBalcao(String data, DefaultTableModel tabela) {
        ...
        try (PreparedStatement pstmt = ConectaBanco.getConnection().prepareStatement(SqlSelection)) {
            ...
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                tabela.addRow(new Object[]{rs.getInt("ped_cod"), "Balcão", "", "", rs.getString("ped_pagamento"), rs.getString("ped_vlr_tot"), rs.getString("ped_stt_canc")});
            }
        } catch (SQLException ex) {
            Logger.getLogger(CancelamentoPedidoModel.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void listarItemPedido(int codigo, DefaultTableModel tabela) {
        ...
        try (PreparedStatement pstmt = ConectaBanco.getConnection().prepareStatement(SqlSelection)) {
            ...
            while (rs.next()) {
                ...
                tabela.addRow(new Object[]{rs.getString("tprd_descr") + meia, rs.getString("prd_descr"), rs.getDouble("item_preco_unit"), rs.getInt("item_quantidade"), rs.getDouble("item_preco_tot"), rs.getString("ped_hr")});
            }
        } catch (SQLException ex) {
            Logger.getLogger(CancelamentoPedidoModel.class.getName()).log(Level.SEVERE, null, ex);
        }
