...
public class AtualizaMovimentoEstoqueModel {
    public boolean atualizaMovimentoEstoque(List<AtualizaMovimentoEstoqueBean> atualizaMovimentoEstoque) {

        for (int i = 0; i < atualizaMovimentoEstoque.size(); i++) {
            try {
                ...
                pstmt.setString(1, atualizaMovimentoEstoque.get(i).getDataMovimento());
                pstmt.setInt(2, atualizaMovimentoEstoque.get(i).getProdutoBean().getCodigo());
                pstmt.setInt(3, atualizaMovimentoEstoque.get(i).getTipoMovimento());
                pstmt.setLong(4, atualizaMovimentoEstoque.get(i).getQuantidadeProduto());

                pstmt.execute();

                if (!atualizaSaldoEstoque(atualizaMovimentoEstoque)) {
                    return false;
                }
    ...
    public boolean atualizaSaldoEstoque(List<AtualizaMovimentoEstoqueBean> atualizaSaldoEstoque) {

        boolean insert = true;

        for (int i = 0; i < atualizaSaldoEstoque.size(); i++) {

            if (atualizaSaldoEstoque.get(i).getProdutoBean().getEstocavel() == 'S') {
                long quantidadeEntrada = 0;
                long quantidadeSaida = 0;
                long saldoAtual = 0;
                long saldoAnterior = 0;
                String dataSaldoEstoque = atualizaSaldoEstoque.get(i).getDataMovimento();
                ...
                try {
                    String SQLPesquisa = "select\n"
                            ...
                            + "WHERE `estq_cod_prod` = '" + atualizaSaldoEstoque.get(i).getProdutoBean().getCodigo() + "'"
                            + " and `estq_dt_movto` = '" + atualizaSaldoEstoque.get(i).getDataMovimento() + "';";
                ...
                try {
                    String SQLPesquisa = "select \n"
                            ...
                            + "where `sdoe_cod_prod` = " + atualizaSaldoEstoque.get(i).getProdutoBean().getCodigo() + "\n "/*+ " AND `sdoe_dt_movto` < '" + pedidoBeans.getData() + "'\n "*/
                    ...
                    if (rs.next()) { //verifico se tem algum registro do produto
                        ...
                        if (dataSaldoEstoque.equals(atualizaSaldoEstoque.get(i).getDataMovimento())) {//se o resultado do rs for a data atual, então contém registo deste produto de hoje
                ...
                if (!insert) {
                    ...
                    try {
                        ...
                        pstm.setString(5, atualizaSaldoEstoque.get(i).getDataMovimento());
                        pstm.setInt(6, atualizaSaldoEstoque.get(i).getProdutoBean().getCodigo());
                    ...
                    try {
                        ...
                        pstm.setString(1, atualizaSaldoEstoque.get(i).getDataMovimento());
                        pstm.setInt(2, atualizaSaldoEstoque.get(i).getProdutoBean().getCodigo());
    ...
    public boolean atualizaMovimentoEstoque(AtualizaMovimentoEstoqueBean atualizaMovimentoEstoque) {
    ...
    public boolean atualizaSaldoEstoque(AtualizaMovimentoEstoqueBean atualizaSaldoEstoque) {

        boolean insert = true;

        

            if (atualizaSaldoEstoque.getProdutoBean().getEstocavel() == 'S') {
                long quantidadeEntrada = 0;
                long quantidadeSaida = 0;
                long saldoAtual = 0;
                long saldoAnterior = 0;
                String dataSaldoEstoque = atualizaSaldoEstoque.getDataMovimento();
                ...
                try {
                    String SQLPesquisa = "select\n"
                            ...
                            + "  `estq_qtd`\n"
                            + "from `pizzaria`.`movto_estoq`"
                            + "WHERE `estq_cod_prod` = '" + atualizaSaldoEstoque.getProdutoBean().getCodigo() + "'"
                            + " and `estq_dt_movto` = '" + atualizaSaldoEstoque.getDataMovimento() + "';";
                ...
                try {
                    String SQLPesquisa = "select \n"
                            ...
                            + "from\n"
                            + "  `pizzaria`.`saldo_estoque` \n"
                            + "where `sdoe_cod_prod` = " + atualizaSaldoEstoque.getProdutoBean().getCodigo() + "\n "/*+ " AND `sdoe_dt_movto` < '" + pedidoBeans.getData() + "'\n "*/
                            + "ORDER BY `sdoe_dt_movto` desc \n"
                            + "limit 2 ;";
                    ...
                    if (rs.next()) { //verifico se tem algum registro do produto
                        ...
                        if (dataSaldoEstoque.equals(atualizaSaldoEstoque.getDataMovimento())) {//se o resultado do rs for a data atual, então contém registo deste produto de hoje
                            if (rs.next()) {//verifico a data anterior da última atualização do registro desse produto
                                dataSaldoEstoque = rs.getString("sdoe_dt_movto");
                                saldoAnterior = rs.getLong("sdoe_qtd_sdo_atu");
                                insert = false;
                ...
                if (!insert) {
                    ...
                    try {
                        ...
                        pstm.setString(5, atualizaSaldoEstoque.getDataMovimento());
                        pstm.setInt(6, atualizaSaldoEstoque.getProdutoBean().getCodigo());
                    ...
                    try {
                        String SQLInsert = "insert into `pizzaria`.`saldo_estoque`\n"
                                ...
                                + "        ?);";
                        PreparedStatement pstm = ConectaBanco.getConnection().prepareStatement(SQLInsert);
                        pstm.setString(1, atualizaSaldoEstoque.getDataMovimento());
                        pstm.setInt(2, atualizaSaldoEstoque.getProdutoBean().getCodigo());
                        pstm.setLong(3, saldoAnterior);
                        pstm.setLong(4, quantidadeEntrada);
                        pstm.setLong(5, quantidadeSaida);
