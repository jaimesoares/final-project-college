...
public class RequisicaoMercadoriaModel {

    public RequisicaoMercadoriaModel() {
    }
    ...
    public void listarTabelaMovimentoEstoque(String data, int codigo, DefaultTableModel tabela, JTextField anterior, JTextField atual) {
        ...
        try (PreparedStatement pstmt = ConectaBanco.getConnection().prepareStatement(SqlSelection)) {
            ...
            Logger.getLogger(RequisicaoMercadoriaModel.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public boolean ajusteEstoque(RequisicaoMercadoriaBean requisicaoMercadoria) {

        try {
            String SQLInserePedido = "insert into `pizzaria`.`requis_estoq`\n"
                    + "            (`req_dt_movto`,\n"
                    + "             `req_setor`,\n"
                    + "             `req_cod_prod`,\n"
                    + "             `req_tipo_movto`,\n"
            ...
            pstmt.setString(1, VerificarData.converteParaSql(requisicaoMercadoria.getDataMovimento()));
            pstmt.setInt(2, requisicaoMercadoria.getCodigoSetor());
            pstmt.setInt(3, requisicaoMercadoria.getProdutoBean().getCodigo());
            pstmt.setInt(4, requisicaoMercadoria.getTipoMovimento());
            pstmt.setLong(5, requisicaoMercadoria.getQuantidadeMovimento());
            pstmt.setInt(6, requisicaoMercadoria.getCodigoFunc());
            pstmt.execute();

            AtualizaMovimentoEstoqueBean novo = new AtualizaMovimentoEstoqueBean();
            novo.setDataMovimento(VerificarData.converteParaSql(requisicaoMercadoria.getDataMovimento()));
            novo.setProdutoBean(requisicaoMercadoria.getProdutoBean());
            novo.setQuantidadeProduto(requisicaoMercadoria.getQuantidadeMovimento());
            novo.setTipoMovimento(requisicaoMercadoria.getTipoMovimento());
            ...
            try {
                ...
                JOptionPane.showMessageDialog(null, "Impossível Cadastrar Requisição de Mercadoria" + ex, "Erro de SQL", 0, new ImageIcon("imagens/cancelar.png"));
